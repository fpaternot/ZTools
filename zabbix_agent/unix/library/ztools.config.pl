...
# Configuracoes globais:
  ...
  #-----------------------------------------------------------------------------
  # Arquivo de configuracao do ZTools:
  my $ZTOOLSConfig = "${BASEDIR}/etc/ztools.conf";
...
MAIN: {
  ...
  my $ztoolsConf = LoadINI( $ZTOOLSConfig );
  
  if( "$Action" eq "SET" ){
    if( $ztoolsConf->{ $KeyName }->{ 'value' } != $KeyValue ){
      $ztoolsConf->{ $KeyName }->{ 'value' } = $KeyValue;
      $ztoolsConf->{ $KeyName }->{ 'change' } = 1;
      $ztoolsConf->{ $KeyName }->{ 'status' } = 1;
      &UpdateINI( $ZTOOLSConfig, $ztoolsConf );
      print "1\n";
    } else {
      print "0\n";
    }
    ...
    if( $KeyName =~ m/all/i ){
      foreach my $KeyValue ( sort( keys( %{ $ztoolsConf }))){
        print "$KeyValue=".$ztoolsConf->{ $KeyValue }->{ 'value' }."\n";
      }
    } else {
      if( defined ( $ztoolsConf->{ $KeyName }->{ 'value' })){
        print "$KeyName=".$ztoolsConf->{ $KeyName }->{ 'value' }."\n";
      } else {
        print "ZBX_NOTSUPPORTED: Parametro de configuracao [$KeyValue] nao encontrado.\n";
      }
    }
